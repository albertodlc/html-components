import { FilterSelect } from '../select/appSelect.js';
...
class FilterConectividad extends HTMLElement {
  ...
  handleEvent(event) {
    if (event.type === 'selectChanged') {
      if (event.srcElement.id === 'region') {
        ...
        // ! Load ZONA
        this.querySelector('#zona').loadList({
          url: getBaseUrl() + '/data/listTwo.json',
        });
      } else if (event.srcElement.id === 'zona') {
        // !Load UTD
        this.querySelector('#utd').loadList({
          url: getBaseUrl() + '/data/listThree.json',
        });
      }
    }
  }
  ...
  render() {
    ...
    this.querySelector('#region').loadList({
      url: getBaseUrl() + '/data/listOne.json',
    });
  }
}
