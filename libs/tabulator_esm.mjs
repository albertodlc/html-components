  ...
  rowsToData(rows) {
    ...
    rows.forEach((row) => {
      ...
      if (hasDataTreeColumnCalcs && row.modules.dataTree?.open) {
        this.rowsToData(dataTree.getFilteredTreeChildren(row)).forEach(
          (dataRow) => {
            data.push(row);
          },
        );
    ...
    if (config && config.children) {
      ...
      config.children.forEach((childRow) => {
        if (childRow instanceof Row) {
          ...
          if (recurse) {
            this.getTreeChildren(childRow, component, recurse).forEach(
              (child) => {
                output.push(child);
              },
            );
...
function datetime$2(cell, onRendered, success, cancel, editorParams) {
  var inputFormat = editorParams.format,
    vertNav = editorParams.verticalNavigation || 'editor',
    DT = inputFormat
      ? this.table.dependencyRegistry.lookup(['luxon', 'DateTime'], 'DateTime')
      : null,
    newDatetime;
...
function adaptable$1(cell, onRendered, success, cancel, params) {
  ...
  lookup = params.editorLookup
    ? params.editorLookup(cell)
    : defaultLookup(cell);
...
class Export extends Module {
  ...
  processColumnGroup(column) {
    var subGroups = column.columns,
      maxDepth = 0,
      title =
        column.definition['title' + this.colVisPropAttach] ||
        column.definition.title;
  ...
  lookupTableStyles() {
    ...
    if (this.cloneTableStyle && window.getComputedStyle) {
      ...
      if (styles.firstRow) {
        ...
        styles.styleRowHeader = styles.firstRow.getElementsByClassName(
  ...
  generateHeaderElement(row, setup, styles) {
    ...
    row.columns.forEach((column) => {
      if (column) {
        ...
        if (
          column.component._column.parent &&
          column.component._column.parent.isGroup
        ) {
          this.mapElementStyles(
        ...
        if (column.component._column.isGroup) {
          ...
          this.mapElementStyles(this.table.columnManager.getElement(), cellEl, [
            'border-bottom',
          ]);
        }
...
function link(cell, formatterParams, onRendered) {
  ...
  if (label) {
    if (formatterParams.urlField) {
      ...
      value = Helpers.retrieveNestedData(
        this.table.options.nestedFieldSeparator,
        formatterParams.urlField,
        data,
      );
    }
...
function datetime$1(cell, formatterParams, onRendered) {
  var DT = this.table.dependencyRegistry.lookup(
    ['luxon', 'DateTime'],
    'DateTime',
  );
...
function toggle(cell, formatterParams, onRendered) {
  var value = cell.getValue(),
    ...
    containEl,
    switchEl,
    onValue = formatterParams.hasOwnProperty('onValue')
      ? formatterParams.onValue
      : true,
    offValue = formatterParams.hasOwnProperty('offValue')
      ? formatterParams.offValue
      : false,
...
function rownum(cell, formatterParams, onRendered) {
  ...
  row.watchPosition((position) => {
    if (formatterParams.relativeToPage) {
      position +=
        table.modules.page.getPageSize() * (table.modules.page.getPage() - 1);
    }
...
function adaptable(cell, params, onRendered) {
  ...
  lookup = params.formatterLookup
    ? params.formatterLookup(cell)
    : defaultLookup(cell);
    ...
    if (typeof formatterParams.valueMap === 'string') {
      valueMap = function (value) {
        return value.map((item) => {
          return Helpers.retrieveNestedData(
            table.options.nestedFieldSeparator,
            formatterParams.valueMap,
            item,
          );
        });
      };
...
function json$1(cell, formatterParams, onRendered) {
  var indent = formatterParams.indent || '\t',
    multiline =
      typeof formatterParams.multiline === 'undefined'
        ? true
        : formatterParams.multiline,
...
class Format extends Module {
  ...
  initializeColumn(column) {
    ...
    if (typeof column.definition.formatterClipboard !== 'undefined') {
      column.modules.format.clipboard = this.lookupTypeFormatter(
        column,
        'Clipboard',
      );
    }
    ...
    if (typeof column.definition.formatterHtmlOutput !== 'undefined') {
      column.modules.format.htmlOutput = this.lookupTypeFormatter(
        column,
        'HtmlOutput',
      );
    }
...
class Import extends Module {
  ...
  transformHeader(headers) {
    ...
    if (this.table.options.importHeaderTransform) {
      headers.forEach((item) => {
        output.push(
          this.table.options.importHeaderTransform.call(
            this.table,
            item,
            headers,
          ),
  ...
  transformData(row) {
    ...
    if (this.table.options.importValueTransform) {
      row.forEach((item) => {
        output.push(
          this.table.options.importValueTransform.call(this.table, item, row),
        );
      });
    } else {
      return row;
  ...
  startMove(e, column) {
    ...
    if (
      ...
      if (
        ...
        this.table.modules.selectRange.selecting === 'column'
      ) {
        return;
      }
    }
...
class Persistence extends Module {
  ...
  setColumnLayout(layout) {
    this.table.columnManager.setColumns(
      this.mergeDefinition(this.table.options.columns, layout, true),
    );
    return true;
  }
...
class ResizeRows extends Module {
  ...
  calcGuidePosition(e, row, handle) {
    var mouseY =
        typeof e.screenY === 'undefined' ? e.touches[0].screenY : e.screenY,
      handleY =
        handle.getBoundingClientRect().y -
      ...
      if (
        ...
        this.resizeObserver = new ResizeObserver((entry) => {
          if (
            ...
            (table.browserMobile &&
              (!table.modules.edit ||
                (table.modules.edit && !table.modules.edit.currentCell)))
          ) {
...
class SelectRow extends Module {
  ...
  initializeRow(row) {
    ...
    if (self.checkRowSelectability(row)) {
      if (
        self.table.options.selectableRows &&
        self.table.options.selectableRows != 'highlight'
      ) {
  ...
  checkRowSelectability(row) {
    if (row && row.type === 'row') {
      return this.table.options.selectableRowsCheck.call(
        this.table,
        row.getComponent(),
      );
    }
...
class Range extends CoreFeature {
  ...
  layout() {
    ...
    if (
      this.table.options.renderHorizontal === 'virtual' &&
    ...
    if (this.overlaps(_vDomLeft, _vDomTop, _vDomRight, _vDomBottom)) {
      ...
      if (this.table.rtl) {
        this.element.style.right =
          topLeftRowEl.offsetWidth -
          topLeftCellEl.offsetLeft -
          topLeftCellEl.offsetWidth +
          'px';
        this.element.style.width =
          topLeftCellEl.offsetLeft +
          topLeftCellEl.offsetWidth -
          bottomRightCellEl.offsetLeft +
          'px';
      } else {
        this.element.style.left =
          topLeftRowEl.offsetLeft + topLeftCellEl.offsetLeft + 'px';
        this.element.style.width =
          bottomRightCellEl.offsetLeft +
          bottomRightCellEl.offsetWidth -
          topLeftCellEl.offsetLeft +
          'px';
      }
      ...
      this.element.style.height =
        bottomRightRowEl.offsetTop +
        bottomRightRowEl.offsetHeight -
        topLeftRowEl.offsetTop +
        'px';
    }
  }
  ...
  atTopLeft(cell) {
    return (
      ...
      cell.column.getPosition() - 1 === this.left
    );
  }
  ...
  atBottomRight(cell) {
    return (
      ...
      cell.column.getPosition() - 1 === this.right
    );
  }
  ...
  occupiesColumn(col) {
    return (
      this.left <= col.getPosition() - 1 && col.getPosition() - 1 <= this.right
    );
  }
  ...
  destroyedGuard(func) {
    if (this.destroyed) {
      console.warn(
        'You cannot call the ' + func + ' function on a destroyed range',
      );
    }
...
class SelectRange extends Module {
  ...
  initializeColumn(column) {
    if (
      ...
      this.options('headerSortClickElement') !== 'icon'
    ) {
      console.warn(
  ...
  updateHeaderColumn() {
    ...
    if (this.rowSelection) {
      ...
      if (this.rowHeader) {
        this.rowHeader.definition.cssClass =
          this.rowHeader.definition.cssClass + ' tabulator-range-row-header';
    ...
    if (
      ...
      if (
        frozenCols.length > 1 ||
        (frozenCols.length === 1 && frozenCols[0] !== this.rowHeader)
      ) {
        console.warn(
    ...
    if (
      ...
      (this.activeRange.occupies(cell) ||
        ((this.selecting === 'row' || this.selecting === 'all') &&
          this.activeRange.occupiesRow(cell.row)))
    ) {
      return;
    }
  ...
  navigate(jump, expand, dir) {
    ...
    if (moved) {
      ...
      rowManagerRect = this.table.rowManager
        .getElement()
        .getBoundingClientRect();
      columnManagerRect = this.table.columnManager
        .getElement()
        .getBoundingClientRect();
      ...
      if (
        !(
          columnRect.left >=
            columnManagerRect.left + this.getRowHeaderWidth() &&
  ...
  findJumpCellLeft(rowPos, colPos) {
    var row = this.getRowByRangePos(rowPos),
      ...
      isStartingCellEmpty = this.isEmpty(
        row.getData()[columns[colPos].getField()],
      ),
      isLeftOfStartingCellEmpty = columns[colPos - 1]
        ? this.isEmpty(row.getData()[columns[colPos - 1].getField()])
        : false,
      targetCols = this.rowHeader
        ? columns.slice(1, colPos)
        : columns.slice(0, colPos),
      jumpCol = this.findJumpCol(
        row,
        targetCols,
        true,
        isStartingCellEmpty,
        isLeftOfStartingCellEmpty,
      );
  ...
  findJumpCellRight(rowPos, colPos) {
    var row = this.getRowByRangePos(rowPos),
      ...
      isRightOfStartingCellEmpty = columns[colPos + 1]
        ? this.isEmpty(row.getData()[columns[colPos + 1].getField()])
        : false,
      jumpCol = this.findJumpCol(
        row,
  ...
  findJumpCellUp(rowPos, colPos) {
    var column = this.getColumnByRangePos(colPos),
      ...
      isStartingCellEmpty = this.isEmpty(
        rows[rowPos].getData()[column.getField()],
      ),
      isTopOfStartingCellEmpty = rows[rowPos - 1]
        ? this.isEmpty(rows[rowPos - 1].getData()[column.getField()])
        : false,
      jumpRow = this.findJumpRow(
        column,
        rows.slice(0, rowPos),
        true,
        isStartingCellEmpty,
        isTopOfStartingCellEmpty,
      );
  ...
  findJumpCellDown(rowPos, colPos) {
    var column = this.getColumnByRangePos(colPos),
      ...
      isBottomOfStartingCellEmpty = rows[rowPos + 1]
        ? this.isEmpty(rows[rowPos + 1].getData()[column.getField()])
        : false,
      jumpRow = this.findJumpRow(
        column,
  ...
  autoScroll(range, row, column) {
    ...
    if (!withinHorizontalView) {
      if (rect.left < view.left) {
        ...
        tableHolder.scrollLeft = Math.min(
          rect.right - tableHolder.clientWidth,
          rect.left - this.getRowHeaderWidth(),
        );
      }
    }
  ...
  layoutRanges() {
    ...
    if (this.table.rtl) {
      this.activeRangeCellElement.style.right =
        activeRowEl.offsetWidth -
        activeCellEl.offsetLeft -
  ...
  getCell(rowIdx, colIdx) {
    ...
    return row
      ? row.getCells(false, true).filter((cell) => cell.column.visible)[colIdx]
      : null;
  }
  ...
  resetRanges() {
    ...
    if (this.table.rowManager.activeRows.length) {
      visibleCells = this.table.rowManager.activeRows[0].cells.filter(
        (cell) => cell.column.visible,
      );
  ...
  selectedRows(component) {
    return component
      ? this.activeRange.getRows().map((row) => row.getComponent())
      : this.activeRange.getRows();
  }

  selectedColumns(component) {
    return component
      ? this.activeRange.getColumns().map((col) => col.getComponent())
...
class GridCalculator {
  ...
  genColumns(data) {
    var colCount = Math.max(
      this.columnCount,
      Math.max(...data.map((item) => item.length)),
    );
  ...
  incrementChar(i) {
    ...
    if (char) {
      if (char !== 'Z') {
        this.columnString[i] = String.fromCharCode(
          this.columnString[i].charCodeAt(0) + 1,
        );
      } else {
    ...
    return new Proxy(this, {
      get: function (target, name, receiver) {
        if (typeof target[name] !== 'undefined') {
          ...
          return target._sheet.table.componentFunctionBinder.handle(
            'sheet',
            target._sheet,
            name,
          );
...
class Sheet extends CoreFeature {
  ...
  getData(full) {
    ...
    if (!full && !this.options('spreadsheetOutputFull')) {
      ...
      rowWidths = output.map(
        (row) => row.findLastIndex((val) => typeof val !== 'undefined') + 1,
      );
...
class ColumnManager extends CoreFeature {
  ...
  setColumns(cols, row) {
    ...
    if (this.table.options.rowHeader) {
      this.rowHeader = new Column(
        this.table.options.rowHeader === true
          ? {}
  ...
  _addColumn(definition, before, nextToColumn) {
    ...
    if (
      before &&
      this.rowHeader &&
      (!nextToColumn || nextToColumn === this.rowHeader)
    ) {
      before = false;
  ...
  getColumnsByFieldRoot(root) {
    ...
    Object.keys(this.columnsByField).forEach((field) => {
      var fieldRoot = this.table.options.nestedFieldSeparator
...
class Layout extends Module {
  ...
  layout(dataChanged) {
    var variableHeight = this.table.columnManager.columnsByIndex.find(
      (column) =>
        column.definition.variableHeight ||
        column.definition.formatter === 'textarea',
    );
  ...
  static registry = {
    ...
    matchElement(element) {
      return TableRegistry.registry.tables.find(function (table) {
        return element instanceof TableRegistry
          ? table === element
